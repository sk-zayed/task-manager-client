          ...
          <v-card-text class="pt-6">
            ...
            <v-card v-if="cardData.tasks" class="ml-8">
              ...
              <v-slide-y-transition class="py-0" group>
                <template>
                  <v-list-item v-for="(task, idx) in cardData.tasks" :key="idx">
                    <v-list-item-action>
                      <v-checkbox
                        v-model="task.done"
...
export default {
      ...
      try {
        if (this.$refs.cardData.validate()) {
          ...
          if (this.cardData._id) {
            ...
            this.$emit("reload");
            utils.showToast("Seems like you have a plan ;)", "success");
          }
        }
      } catch (error) {
  ...
  mounted() {
    // this.cardData = store.getters.cardObject;
    console.log("mounted", this.$store.boards.cardObject);
  },
  created() {
    this.cardData = store.getters.cardObject;
    console.log("created", this.cardData);
  },
};
</script>
